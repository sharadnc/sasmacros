/**************************************************************************************************************
* Macro_Name: compare_one_record_portal
*
...
*
* Usage: %compare_one_record_portal(data=,base=,keyvar1-5=,keyval1-5=,outfile=_webout)
*
...
*           in1_not_in2
*            Number of variables in data not in base
*           in2_not_in1
*            Number of variables in base not in data
*
* Example:
*
* Modules_called: None
...
*-------------------------------------------------------------------------------------------------------------*
* 02/24/2012  | Michael Gilman|  | Initial creation.                                                          *
* 07/21/2012  | Michael Gilman|  | Now print out all variable variables when new record.                      *
*-------------------------------------------------------------------------------------------------------------*
* HeaderEnd:                                                                                                  *
**************************************************************************************************************/

%macro compare_one_record_portal(base=,data=,keyvar1=,keyvar2=,keyvar3=,keyvar4=,keyvar5=,
...
%if %qupcase(%superq(outfile))=_WEBOUT %then
 ...
 %do;
    ...
    %let errormsg=ERROR: Data set &data does not exist;

    %goto ERREXIT;

 %end;

 %if %sysfunc(exist(&base))=0 %then
 %do;

    %let errormsg=ERROR: Data set &base does not exist;
 ...
 title1 "Compare one record from &data and &base";
 ...
 %do i=1 %to &numberOfKeyvars;
    ...
    %do;
       ...
       %let errormsg=ERROR: Key variable &keyvar does not exist in &data;
    ...
    %do;
       ...
       %let errormsg=ERROR: Key variable &keyvar does not exist in &base;
 ...
 %do;
    ...
    %let errormsg=ERROR: &base and &data have no variables in common other than the Key variable;
 ...
 %do i=1 %to &numberOfKeyvars;
    ...
    %do;
       ...
       %let errormsg=ERROR: Key variable &&keyvar&i in data set &data is type &&keyvartype1_&i but is type &&keyvartype2_&i in data set
 &base;
 ...
 %do i=1 %to 5;
    ...
    %if &&keyvar&i ne %str() %then
    %do;

       %if &i>1 %then %let comma=%str(,);

       %let keyvars=&keyvars &comma &&keyvar&i;
 ...
 %do i=1 %to 5;
    ...
    %if &&keyval&i ne %str() %then
    %do;

       %if &i>1 %then %let comma=%str(,);

       %let keyvals=&keyvals &comma &&keyval&i;
 ...
 title2 "Key variable(s): &keyvar";
 title3 "Key value(s): &keyvals";

 %let dsid1=%sysfunc(open(&data(where=(&wcls))));

 %let dsid2=%sysfunc(open(&base(where=(&wcls))));
 ...
 %if &obs2=0 %then %let new_record=1;
 %else %let new_record=0;


 %if &obs1=0 %then
 %do;

    %let errormsg=ERROR: No record found in data set &data having a value for the key variable(s);

	data temp;
	 Message="ERROR: Record not found in &base";
	run;

	title4 "ERROR: Record not found in &base";
 ...
 %do;
    ...
    %let errormsg=ERROR: Data set &data has &obs1 records for the key variable(s);

    %goto ERREXIT;

 %end;

 %if &new_record %then %goto NEWREC;

 %if &obs2>1 %then
 %do;

    %let errormsg=ERROR: Data set &base has &obs2 records for the key variable(s);
...
%NEXT:
 ...
 data temp;
  ...
  Message="Number of variables in &data not in &base";value="&in1_not_in2";output;
  Message="Number of variables in &base not in &data";value="&in2_not_in1";output;
 ...
 %do;
    ...
    title4 "Variable values that don't match";
...
%ERREXIT:
 ...
 %do;
	...
	title4 "Record does not exist in &base dataset.";

    data temp(rename=(__nn=Variable __vv=Value));

     set &data;
     where &wcls;

     array __c{*} $32 _character_;
     array __n{*} 8 _numeric_;
 ...
 %do;
    ...
    title4 "&errormsg";
	  ...
	  title5 "Record does not exist in &base dataset.";
 ...
 %do;
    ...
    title1 "There are &conflicting_types_cnt variables in common that have conflicting types";

    proc print data=conflicting_types label noobs;
    label vartype1="&data" vartype2="&base";
    run;
 ...
 %do;
    ...
    title1 Variables in data set &data but not in &base;
 ...
 %do;
    ...
    title1 Variables in data set &base but not in &data;
 ...
 %put INFO: Number of variables in &data not in &base: &in1_not_in2;
 %put INFO: Number of variables in &base not in &data: &in2_not_in1;
 %put;

%mend compare_one_record_portal;